...
import TP60WebChat from "./TP60WebChat";
...
export default function DeliveryContent() {
  ...
  async function handleEmailSubmit(event: React.FormEvent) {
    ...
    try {
      await fetch(`https://script.google.com/macros/s/AKfycbySrZYxI-NNnXfxY1jXOqHgT2HQi4zst2Fgte6FXTeymat_W_r0o1E3P83EfnVCjEk0/exec?action=delivery_email&email=${encodeURIComponent(email)}&store=PL601`, { method: "GET", mode: "no-cors" });
    } finally {
  ...
  useEffect(() => {
    ...
    fetch("https://milestone-1-demo.vercel.app/api/catalog?store=TP60", { signal: controller.signal })
  ...
  return <main className={styles.main}>
    ...
    <section className={styles.hero}>
      <Image src="/banners/Planet60_Delivery.webp" alt="The Planet 60 delivery menu" width={1200} height={500} priority sizes="100vw" />
      <div><p>The Planet 60</p><h1>Delivery Menu</h1><span>Browse the shared product catalog. The store confirms current availability and delivery details before an order is accepted.</span></div>
    </section>
    ...
    <section className={styles.howToOrder} aria-labelledby="how-to-order-title">
      <div><p>HOW TO ORDER</p><h2 id="how-to-order-title">Order with the The Planet 60 dispatcher</h2><span>LIVE ORDER connects you with the The Planet 60 dispatcher.</span></div>
      <ol>
        ...
        <li><strong>Confirm with the dispatcher</strong><span>The The Planet 60 dispatcher confirms availability, delivery details, and next steps.</span></li>
      </ol>
    </section>
      ...
      <div className={styles.catalog}>
        ...
        <div className={styles.grid}>{visible.map((product) => <article className={styles.card} key={product.publicProductId}>
          <button type="button" className={styles.imageButton} onClick={() => setSelected(product)} aria-label={`View details for ${product.name}`}>
            {product.images[0] ? <Image src={product.images[0]} alt={`${product.name} on The Planet 60 delivery menu`} fill sizes="(max-width: 640px) 50vw, 280px" unoptimized /> : <span>The Planet 60</span>}
          </button>
    ...
    <section className={styles.updates} aria-labelledby="delivery-updates-title">
      <div><p>DELIVERY UPDATES</p><h2 id="delivery-updates-title">Get store delivery updates</h2><span>Enter your email to keep The Planet 60 delivery notification active.</span></div>
    ...
    <div className={styles.ctaSection}><p>Visit us in-store at <strong>1098 Peter Robertson Blvd #10, Brampton</strong> — open <strong>24 hours</strong>. Call <strong>(437) 230-6974</strong>.</p></div>
