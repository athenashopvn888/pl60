...
function previewStatus(): StaffStatus {
  return {
    store: { code: "PL601", name: "The Planet 60" }, dayKey: "2026-07-23", submissions: [],
...
async function redrawImage(file: File) {
  ...
  return new File([blob], "pl6-staff-photo.jpg", { type: "image/jpeg", lastModified: Date.now() });
}
  ...
  if (!authenticated) return (
    <main className={styles.shell}>
      <section className={styles.loginCard} aria-labelledby="login-title">
        <div className={styles.brandMark}>T60</div>
        <p className={styles.eyebrow}>PL601 · Staff only</p>
        <h1 id="login-title">Today’s store task</h1>
        <p className={styles.intro}>Enter The Planet 60 staff PIN.</p>
  ...
  return (
    <main className={styles.shell}>
      <header className={styles.header}>
        <div><p className={styles.eyebrow}>PL601 · Staff</p><h1>Today’s store task</h1><p>{status?.dayKey}</p></div>
